...
- Writes: a phase plan file (`.planning/PLAN.md`, written by gsd-planner) and
...
2. **Delegate to gsd-planner (second).** Invoke it via the Agent tool, passing
   ...
   and the phase's CONTEXT. Instruct it to write the plan to `.planning/PLAN.md`.
...
3. Delegate to gsd-planner (step 2 above); it writes `.planning/PLAN.md`.
...
- `PLAN.md` is per-phase and overwritten each phase (mirrors CONTEXT.md). The
